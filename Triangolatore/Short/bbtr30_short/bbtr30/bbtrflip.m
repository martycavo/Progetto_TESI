function [] = bbtrflip (T1,T2,Border);
%
% [] = bbtrflip (T1,T2,Border)
%
% This function locally modify an existing grid, checking if a border must
% be flipped (*see attachments) to recreate a Delaunay triangulation (*see
...
% in positive case flips the border. Then the function checks if newborn
% triangles must be flipped again (the functionis recursive)
%
% Inputs of this function are:
%
% T1: the triangle that must be controlled
% T2: this is the other triangle that may be flipped. "The flip wawe" goes
%     in this direction
% Border: the reference to the Border that may be flipped
...
if T2 ~= -1
    ...
    FlipPosition = check_if_in_circle (V(FarVertex2,1),V(FarVertex2,2),TInfo(T1).Circumcenter,TInfo(T1).Circumradius);
    
    if FlipPosition <= 0 %Flip borders
        ...
        % Flip actual grid
        bbtrflip (NewTriangle1,Neighbour24,Border24);
        bbtrflip (NewTriangle2,Neighbour23,Border23);
